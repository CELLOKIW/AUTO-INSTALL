...
display_welcome() {
  ...
  echo -e "${BLUE}[+]                 © XELLO ALEXSANDER                [+]${NC}"
  echo -e "${BLUE}[+]                                                 [+]${NC}"
  echo -e "${RED}[+] =============================================== [+]${NC}"
  echo -e ""
...
check_token() {
  echo -e "                                                       "
  echo -e "${BLUE}[+] =============================================== [+]${NC}"
  echo -e "${BLUE}[+]          LICENSE BY XELLO ALEXSANDER               [+]${NC}"
  echo -e "${BLUE}[+] =============================================== [+]${NC}"
  echo -e "                                                       "
  echo -e "${YELLOW}MASUKAN AKSES TOKEN :${NC}"
  read -r USER_TOKEN

  if [ "$USER_TOKEN" = "xello" ]; then
    echo -e "${GREEN}AKSES BERHASIL${NC}}"
  else
    echo -e "${GREEN}BUY / ASK XELLOALEXSANDER IF YOU DON'T KNOW THE TOKEN ${NC}"
    exit 1
  fi
  clear
}
...
$node_name
INSTALLED WITH LOVE BY XELLOALEXSANDER 
$locid
https
$node_name
yes
